...
interface UseCrouchProneProps {
    capsule: Capsule;
    clock: Clock;
    crouchHoldStart: React.MutableRefObject<number | null>;
    ...
    proneAttempted: React.MutableRefObject<boolean>;
...
export const crouchAndProne = (props: UseCrouchProneProps) => {
    ...
    const { crouchHoldStart, currentHeight, proneAttempted, wasCrouchPressed } = props;
    ...
    if (isCrouchPressed && !wasCrouchPressed.current) {
        crouchHoldStart.current = elapsedTime; // Start timer for prone
        proneAttempted.current = false; // Reset prone attempt flag
    ...
    if (isCrouchPressed && wasCrouchPressed.current) {
        const elapsed = elapsedTime - (crouchHoldStart.current ?? 0);

        // If the button is held long enough to go prone (350ms)
        if (elapsed >= 0.35 && !proneAttempted.current) {
